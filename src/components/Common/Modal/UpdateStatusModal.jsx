...
import { useUpdateStatusMutation } from "../../../feature/enroll/enrollSlice";
...
const UpdateStatusModal = ({ itemId, status, open, setOpen }) => {
  ...
  const [updateEnroll] = useUpdateStatusMutation();

  const updateStatus = async () => {
    const status = {
      status: statusUpdate,
    };
    // console.log("final test: ", status, itemId, auth?.access_token);
      ...
      <Box sx={style}>
        <div className="p-5 ">
          <div className="w-full ">
            ...
            <p className="text-center text-xl font-medium">Are you sure to make it <span className="font-semibold">{statusUpdate}</span>?</p>
          </div>
